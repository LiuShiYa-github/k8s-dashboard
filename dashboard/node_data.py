...
from dashboard import auth_check  # 使用时间格式化
import re
...
def node_info(core_api, n_name=None):
    ...
    for node in core_api.list_node().items:
        ...
        node_info[node_name]["create_time"] = auth_check.timestamp_format(node.metadata.creation_timestamp)
...
def node_resource(core_api, n_name=None):
    ...
    for node in core_api.list_node().items:
        node_name = node.metadata.name
        node_resource[node_name] = {"allocatable_cpu": "", "capacity_cpu": "", "allocatable_memory": "",
                                    "capacity_memory": "",
        ...
        node_resource[node_name]["allocatable_ephemeral_storage"] = round(int(allocatable_storage) / 1024 / 1024 / 1024,
                                                                          2)
        ...
        for c in pod.spec.containers:
            ...
            if c.resources.requests is not None:
                if "cpu" in c.resources.requests:
                    ...
                    node_resource[node_name]['cpu_requests'] = round(
                        node_resource[node_name]['cpu_requests'] + cpuUnitToF(cpu_request), 2)
                if "memory" in c.resources.requests:
                    memory_request = c.resources.requests["memory"]
                    node_resource[node_name]['memory_requests'] = round(
                        node_resource[node_name]['memory_requests'] + memoryUnitToG(memory_request), 2)
            # 资源限制
            if c.resources.limits is not None:
                if "cpu" in c.resources.limits:
                    cpu_limit = c.resources.limits["cpu"]
                    node_resource[node_name]['cpu_limits'] = round(
                        node_resource[node_name]['cpu_limits'] + cpuUnitToF(cpu_limit), 2)
                if "memory" in c.resources.limits:
                    memory_limit = c.resources.limits["memory"]
                    node_resource[node_name]['memory_limits'] = round(
                        node_resource[node_name]['memory_limits'] + memoryUnitToG(memory_limit), 2)
        # 添加Pod数量
